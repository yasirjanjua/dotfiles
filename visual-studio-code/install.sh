...
code -v > /dev/null
if [[ $? -eq 0 ]];then
    read -r -p "you want to install VSC extensions? [y|N] " configresponse
    ...
    if [[ $configresponse =~ ^(y|yes|Y) ]];then
        ...
        if [[ $backupresponse =~ ^(n|no|N) ]];then
            ...
            cp $APPDATA/Code/User/settings.json $APPDATA/Code/User/settings.backup.json
            ok "Your previous config has been saved to: $APPDATA/Code/User/settings.backup.json"
        fi
